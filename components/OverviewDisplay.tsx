...
function BoldStat({ score, percentile }: { score: number; percentile: number }) {
  const ratio = (100 / (100 - percentile)).toFixed(1);
  return (
...
function ScoreContext({ metrics, pillar, score }: {
  ...
  const midMetrics = sorted.slice(1, sorted.length - 1).slice(0, 2);

  return (
    <div>
      <p className="text-sm text-zinc-600 leading-relaxed mb-4">{sentence}</p>
...
export default function OverviewDisplay({ result, onReset, onGoToAnalysis }: OverviewDisplayProps) {
  const [activeTab, setActiveTab] = useState<"Overall" | "Harmony" | "Angularity" | "Dimorphism">("Overall");

  const getScore = (id: string) => result.metrics.find(r => r.definition.id === id)?.score ?? 5.0;

  const harmonyMetrics = useMemo(() => {
    return HARMONY_CATEGORIES.flatMap(cat =>
  ...
  const finalPercentile = getPercentile(finalScore);
  ...
  const pillarData = {
    ...
    Harmony: {
      score: harmonyScore,
      metrics: harmonyMetrics.map(m => ({ name: m.definition.name, score: m.score! })),
    },
    Angularity: {
      score: angularityScore,
      metrics: angularityItems,
    },
    Dimorphism: {
      score: dimorphismScore,
      metrics: [...dimorphismGeoItems, ...dimorphismVisionItems],
    },
  };
        ...
        <div className="max-w-7xl mx-auto px-6 py-6 space-y-4">
          ...
          <div className="grid grid-cols-4 gap-4">
            {pillars.map(p => (
              <button
                key={p.label}
                onClick={() => setActiveTab(p.label as any)}
              ...
              >
                <p className={`text-xs font-mono uppercase tracking-widest mb-3 ${activeTab === p.label ? "text-zinc-400" : "text-zinc-400"}`}>
                  {p.label}
                </p>
                <p className="text-3xl font-semibold mb-1" style={{ color: activeTab === p.label ? "#fff" : getScoreColor(p.score) }}>
                  {p.score.toFixed(2)}
                </p>
                <p className={`text-xs ${activeTab === p.label ? "text-zinc-400" : "text-zinc-400"}`}>
          ...
          <div className="bg-white rounded-2xl border border-zinc-200 overflow-hidden">
            ...
            <div className="grid grid-cols-3 gap-0">
              {/* Photo */}
              <div className="relative overflow-hidden border-r border-zinc-100" style={{ height: 420 }}>
                <img src={result.imageUrl} alt="Your photo" className="w-full h-full object-cover" />
                <div className="absolute bottom-0 left-0 right-0 px-4 py-3 bg-gradient-to-t from-black/70 to-transparent">
                  <p className="text-white text-sm font-medium">{activeTab} Score</p>
              ...
              <div className="p-6 overflow-y-auto" style={{ maxHeight: 420 }}>
                <div className="mb-5">
